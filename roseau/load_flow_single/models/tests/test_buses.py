...
def test_res_voltages():
    bus = Bus(id="bus")
    bus._res_potential = 230 + 0j

    assert np.allclose(bus.res_voltage.m, 230 * np.sqrt(3.0) + 0j)
    assert bus.res_voltage_level is None
    bus.nominal_voltage = 400  # V
    assert np.allclose(bus.res_voltage_level.m, 230 / 400 * np.sqrt(3))


def test_res_violated():
    bus = Bus(id="bus")
    bus._res_potential = 230 + 0
